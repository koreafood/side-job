...
SERVER_HOST="${SERVER_HOST:-192.168.0.63}"
SERVER_USER="${SERVER_USER:-ubuntu}"
REMOTE_PATH="${REMOTE_PATH:-/srv/lalawon/app}"
DOMAIN="${DOMAIN:-home.lala.dedyn.io}"
API_PORT="${API_PORT:-8001}"
SERVICE_NAME="${SERVICE_NAME:-lalawon-api}"
SITE_NAME="${SITE_NAME:-lalawon}"
API_ORIGIN=""
API_ORIGIN_SET="false"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --api-origin) API_ORIGIN="$2"; API_ORIGIN_SET="true"; shift 2 ;;
...
if [[ "${DOMAIN}" == "your-domain.example" || -z "${DOMAIN}" ]]; then
  SERVER_NAME="${SERVER_HOST}"
  LISTEN_LINE="listen 80 default_server;"
fi
if [[ "${API_ORIGIN_SET}" != "true" ]]; then
  if [[ "${SERVER_NAME}" == "${SERVER_HOST}" ]]; then
    API_ORIGIN="http://${SERVER_HOST}"
  else
...
if [[ -z "${SERVER_HOST}" ]]; then
  echo "사용법: scripts/deploy.sh --host <SERVER_HOST> [--user <USER>] [--path <REMOTE_PATH>] [--domain <DOMAIN>] [--api-port <PORT>] [--api-origin <URL>] [--enable-ssl true|false] [--certbot-email <EMAIL>] [--sudo-pass <SUDO_PASSWORD>] [--ssh-pass <SSH_PASSWORD>]"
  exit 1
fi
...
if [[ -n "${SUDO_PASS}" ]]; then
  run_sudo "mkdir -p '${REMOTE_PATH}/dist' '${REMOTE_PATH}/api' '${REMOTE_PATH}/api/uploads' && sudo -S -p '' touch '${REMOTE_PATH}/api/app.db' && sudo -S -p '' chown -R '${SERVER_USER}:${REMOTE_GROUP}' '${REMOTE_PATH}'"
else
  "${SSH_CMD[@]}" "${remote}" "sudo mkdir -p '${REMOTE_PATH}/dist' '${REMOTE_PATH}/api' '${REMOTE_PATH}/api/uploads' && sudo touch '${REMOTE_PATH}/api/app.db' && sudo chown -R '${SERVER_USER}:${REMOTE_GROUP}' '${REMOTE_PATH}'"
fi
...
echo "백엔드 코드 전송"
"${SCP_CMD[@]}" ./api/*.py "${remote}:${REMOTE_PATH}/api/"
...
"${SSH_CMD[@]}" "${remote}" "cd '${REMOTE_PATH}' && python3 -m venv .venv && . .venv/bin/activate && pip install -U pip && pip install -r api/requirements.txt"
...
cat <<'NGINX_CONF' | sed -e "s|__SERVER_NAME__|${SERVER_NAME}|g" -e "s|__LISTEN__|${LISTEN_LINE}|g" -e "s|__REMOTE_PATH__|${REMOTE_PATH}|g" -e "s|__API_PORT__|${API_PORT}|g" > "${NGINX_LOCAL}"
server {
    __LISTEN__
